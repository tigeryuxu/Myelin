...
D = -core;  % EDT
mask = imregionalmax(D);   % Extended minima
...
%% THICKEN
%cb = bwmorph3(bw, 'thicken', diameter);  % Then thicken the edges back to the original ledge
cb = imdilate(bw, strel('disk', diameter));  % Then thicken the edges back to the original ledge
