...
for i in range(len(onlyfiles_mask)):  
    ...
    #size = int((size_whole * scale) / 0.6904) # 4775 and 6157 for the newest one
    size = int((size_whole)) # 4775 and 6157 for the newest one
    #input_arr = resize(input_arr,size_h=size_whole, size_w=size_whole, method=Image.BICUBIC)  
    ...
    #DAPI_arr = readIm_counter_DAPI(DAPI_path,onlyfiles_DAPI, counter[i])
    #DAPI_arr = resize(DAPI_arr,size_h=size, size_w=size, method=Image.BICUBIC)
    #DAPI_tmp = np.asarray(DAPI_arr, dtype=float)
    DAPI_size = round(radius * radius * math.pi);  
    DAPI_tmp, total_matched_DAPI, total_DAPI = pre_process(input_arr, counter[i], DAPI_size, name=onlyfiles_mask[counter[i]])
