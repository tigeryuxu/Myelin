...
def run_analysis_QL(s_path, sav_dir, input_path, checkpoint,
    ...
    #    input_arr = input_arr[0:1440, 0:1920, 2]
        ...
        if CLAHE == 1:
            ...
            input_arr_tmp[:,:,0] = input_arr[:, :, 0]
            input_arr_tmp[:,:,2] = input_arr[:, :, 2]
        ...
        DAPI_tmp,  total_matched_DAPI, total_DAPI = pre_process_QL(input_arr_tmp, counter[i], DAPI_size, name=filename, sav_dir=sav_dir)
        ...
        while N < len(cc):  
            ...
            #DAPI_crop, coords = adapt_crop_DAPI_ARRAY(tmp, DAPI_idx, length=len_x, width=width_x)          
