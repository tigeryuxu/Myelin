...
input_path='C:/Users/Tiger/Documents/Tiger 2015/Antel Lab/Myelin Quantification/Source/DAPIfind/10) Super-resolution/MASKS_of_new_image_with_FIBERS/Input_images/'
DAPI_path='C:/Users/Tiger/Documents/Tiger 2015/Antel Lab/Myelin Quantification/Source/DAPIfind/10) Super-resolution/MASKS_of_new_image_with_FIBERS/DAPI_masks/'
mask_path='C:/Users/Tiger/Documents/Tiger 2015/Antel Lab/Myelin Quantification/Source/DAPIfind/10) Super-resolution/MASKS_of_new_image_with_FIBERS/Fiber_masks/'
...
while i < len(counter): #length of your filename      
      ...
      while N < len(cc):  
          ...
          """ Concatenate DAPI_mask """
          expanded_CROP = np.expand_dims(DAPI_crop, axis=-1)
          input_crop = np.append(input_crop, expanded_CROP, axis=-1)
          
          concate_input = np.zeros([1024, 640, 6])
          
          concate_input[:, :, 0:4] = input_crop
          concate_input[:, :, 4:6] = combined
          
                      
          plt.figure('Batched'); plt.clf()
          plt.subplot(211); plt.imshow(concate_input[:, :, 0:3]/255);
          true_m = np.argmax((concate_input[:, :, 4:6]).astype('uint8'), axis=-1)  
          plt.subplot(212); plt.imshow(true_m, vmin = 0, vmax = 1); plt.pause(0.05)             

          plt.figure('DAPI'); plt.clf();
          plt.imshow(DAPI_crop);
          plt.pause(0.05)
          """ Saves image """        
          # Saving the objects:
